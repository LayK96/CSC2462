...
#include <Adafruit_GFX.h>    // Go to Sketch Tab -> Include Library -> Manage Libraries -> Search Adafruit_GFX and install
#include <TouchScreen.h>     // Go to Sketch Tab -> Include Library -> Manage Libraries -> Search TouchScreen and install
#include "pitches.h"         //Included in folder
#include <MCUFRIEND_kbv.h>    // Go to Sketch Tab -> Include Library -> Manage Libraries -> Search MCUFRIEND_kbv and install
...
//Setup code that sends game setup to the Arduino to display on screen
void setup()
{
  initTft(tft);
...
int selection = -1;
//Loop that lets player interact with game
void loop(void)
{
...
/**
 Fucntion to determine positions that get brick and position colors
*/
...
int readUiSelection(game_type * game, game_state_type * state, const int16_t lastSelected ) {
  ...
  //Min pressure to interact with screen
